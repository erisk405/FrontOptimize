...
impl TypeScriptAnalyzer {
                ...
                for specifier in &import_decl.specifiers {
                    match specifier {
                        ImportSpecifier::Named(named) => {
                            ...
                            imported_identifiers.insert(
                                local_name.clone(),
                                (named.span, source.clone())
                            );
                        }
    ...
    fn check_missing_awaits_in_stmt(
        ...
        match stmt {
            Stmt::Block(block) => {
                self.check_missing_awaits_in_block(block, async_functions, in_async_context, issues);
            }
            Stmt::Expr(expr) => {
                self.check_missing_awaits_in_expr(&expr.expr, async_functions, in_async_context, issues);
            }
            Stmt::Decl(decl) => {
            ...
            Stmt::Return(ret) => {
                if let Some(arg) = &ret.arg {
                    self.check_missing_awaits_in_expr(arg, async_functions, in_async_context, issues);
                }
            }
            Stmt::If(if_stmt) => {
                self.check_missing_awaits_in_expr(&if_stmt.test, async_functions, in_async_context, issues);
                self.check_missing_awaits_in_stmt(&if_stmt.cons, async_functions, in_async_context, issues);
                if let Some(alt) = &if_stmt.alt {
                    self.check_missing_awaits_in_stmt(alt, async_functions, in_async_context, issues);
                }
            }
            Stmt::While(while_stmt) => {
                self.check_missing_awaits_in_expr(&while_stmt.test, async_functions, in_async_context, issues);
                self.check_missing_awaits_in_stmt(&while_stmt.body, async_functions, in_async_context, issues);
            }
            Stmt::For(for_stmt) => {
                if let Some(init) = &for_stmt.init {
                    match init {
                        VarDeclOrExpr::VarDecl(var) => {
                            for decl in &var.decls {
                                if let Some(init) = &decl.init {
                                    self.check_missing_awaits_in_expr(init, async_functions, in_async_context, issues);
                                }
                            }
                        }
                        VarDeclOrExpr::Expr(expr) => {
                            self.check_missing_awaits_in_expr(expr, async_functions, in_async_context, issues);
                        }
                    }
                }
                if let Some(test) = &for_stmt.test {
                    self.check_missing_awaits_in_expr(test, async_functions, in_async_context, issues);
                }
                if let Some(update) = &for_stmt.update {
                    self.check_missing_awaits_in_expr(update, async_functions, in_async_context, issues);
                }
                self.check_missing_awaits_in_stmt(&for_stmt.body, async_functions, in_async_context, issues);
            }
            Stmt::Switch(switch) => {
                self.check_missing_awaits_in_expr(&switch.discriminant, async_functions, in_async_context, issues);
                for case in &switch.cases {
                    if let Some(test) = &case.test {
                        self.check_missing_awaits_in_expr(test, async_functions, in_async_context, issues);
                    }
            ...
            Stmt::Try(try_stmt) => {
                self.check_missing_awaits_in_block(&try_stmt.block, async_functions, in_async_context, issues);
                if let Some(handler) = &try_stmt.handler {
                    self.check_missing_awaits_in_block(&handler.body, async_functions, in_async_context, issues);
                }
                if let Some(finalizer) = &try_stmt.finalizer {
                    self.check_missing_awaits_in_block(finalizer, async_functions, in_async_context, issues);
                }
            }
            _ => {}
        }
    ...
    fn check_missing_awaits_in_expr(
        ...
        match expr {
            Expr::Call(call) => {
                ...
                if let Some(callee_expr) = call.callee.as_expr() {
                    let is_async_call = match &**callee_expr {
                        Expr::Ident(ident) => {
                            async_functions.contains(&ident.sym.to_string())
                        }
                        Expr::Member(member) => {
                    ...
                    for arg in &call.args {
                        self.check_missing_awaits_in_expr(&arg.expr, async_functions, in_async_context, issues);
                    }
                }
            }
            Expr::Await(_) => {
            ...
            Expr::Arrow(arrow) => {
                match &*arrow.body {
                    BlockStmtOrExpr::BlockStmt(block) => {
                        self.check_missing_awaits_in_block(block, async_functions, arrow.is_async, issues);
                    }
                    BlockStmtOrExpr::Expr(expr) => {
                        self.check_missing_awaits_in_expr(expr, async_functions, arrow.is_async, issues);
                    }
                }
            }
            Expr::Bin(bin) => {
                self.check_missing_awaits_in_expr(&bin.left, async_functions, in_async_context, issues);
                self.check_missing_awaits_in_expr(&bin.right, async_functions, in_async_context, issues);
            }
            Expr::Unary(unary) => {
                self.check_missing_awaits_in_expr(&unary.arg, async_functions, in_async_context, issues);
            }
            Expr::Assign(assign) => {
                self.check_missing_awaits_in_expr(&assign.right, async_functions, in_async_context, issues);
            }
            Expr::Array(array) => {
                for elem in &array.elems {
                    if let Some(elem) = elem {
                        self.check_missing_awaits_in_expr(&elem.expr, async_functions, in_async_context, issues);
                    }
                }
            }
            Expr::Object(obj) => {
                for prop in &obj.props {
                    match prop {
                        PropOrSpread::Prop(prop) => {
                            match &**prop {
                                Prop::KeyValue(kv) => {
                                    self.check_missing_awaits_in_expr(&kv.value, async_functions, in_async_context, issues);
                                }
                                Prop::Method(method) => {
                                    if let Some(body) = &method.function.body {
                                        self.check_missing_awaits_in_block(
                        ...
                        PropOrSpread::Spread(spread) => {
                            self.check_missing_awaits_in_expr(&spread.expr, async_functions, in_async_context, issues);
                        }
                    }
                }
            }
            Expr::Cond(cond) => {
                self.check_missing_awaits_in_expr(&cond.test, async_functions, in_async_context, issues);
                self.check_missing_awaits_in_expr(&cond.cons, async_functions, in_async_context, issues);
                self.check_missing_awaits_in_expr(&cond.alt, async_functions, in_async_context, issues);
            }
            Expr::Paren(paren) => {
                self.check_missing_awaits_in_expr(&paren.expr, async_functions, in_async_context, issues);
            }
            Expr::Member(member) => {
                self.check_missing_awaits_in_expr(&member.obj, async_functions, in_async_context, issues);
            }
            Expr::New(new) => {
                if let Some(args) = &new.args {
                    for arg in args {
