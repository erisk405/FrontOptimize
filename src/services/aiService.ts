...
export class AnthropicProvider implements AIProvider {
  ...
  async generateRecommendations(
    ...
    console.log("[AnthropicProvider] Starting generateRecommendations", {
      model: this.model,
    ...
    return this.executeWithRetry(async () => {
      ...
      if (cancellationToken?.isCancellationRequested) {
        console.log("[AnthropicProvider] Request cancelled before starting");
        throw new Error("Request cancelled by user");
      }

      console.log("[AnthropicProvider] Making API request to Anthropic");
      ...
      const response = await axios.post(
        "https://api.anthropic.com/v1/messages",
        {
          ...
          max_tokens: 4000,
          system:
            "You are an expert Angular developer with deep knowledge of TypeScript, RxJS, Angular best practices, CSS optimization, and performance tuning. Analyze code issues carefully and provide specific, actionable recommendations with clear explanations. Focus on practical solutions that improve code quality, performance, and maintainability. Always return valid JSON responses when requested.",
          messages: [
            {
              role: "user",
              content: prompt,
        ...
        },
        {
          ...
          signal: cancellationToken
            ? this.createAbortSignal(cancellationToken)
            : undefined,
        },
      );
      ...
      if (response.data.content && response.data.content.length > 0) {
        ...
        console.log("[AnthropicProvider] Successfully extracted text", {
          textLength: text.length,
        });
        return text;
      } else if (response.data.completion) {
        // Fallback for older API format
        console.log("[AnthropicProvider] Using legacy completion format");
        return response.data.completion;
      } else {
        console.error(
          "[AnthropicProvider] Invalid response format",
          response.data,
        );
    ...
    for (let attempt = 0; attempt < this.maxRetries; attempt++) {
      try {
        ...
        if (axios.isAxiosError(error) && error.response?.status === 404) {
          console.error(
            "Anthropic API 404 error. Response:",
            error.response.data,
          );
          throw new Error(
            "Anthropic API endpoint not found. Please check if your API key has access to the Claude models.",
          );
        }
        ...
        console.log(
          `AI request failed (attempt ${attempt + 1}/${this.maxRetries}), retrying in ${delay}ms...`,
        );
...
export class AIService {
  ...
  async initializeProvider(): Promise<void> {
    ...
    console.log("[AIService] Initializing provider", { providerType, model });

    // Get API key from SecretStorage
    const apiKey = await this.getApiKey();

    if (!apiKey) {
      console.error("[AIService] API key not found");
      throw new Error(
        "API key not configured. Please use the 'Configure API Key' command to set your API key.",
      );
    }

    console.log("[AIService] API key found", {
    ...
    console.log("[AIService] Timeout configured", {
      timeoutSeconds,
      timeoutMs,
    });
    ...
    if (providerType === "openai") {
      console.log("[AIService] Creating OpenAI provider");
      this.provider = new OpenAIProvider(apiKey, model, timeoutMs);
    } else if (providerType === "anthropic") {
      console.log("[AIService] Creating Anthropic provider");
      this.provider = new AnthropicProvider(apiKey, model, timeoutMs);
    } else {
      console.error("[AIService] Unsupported provider type", { providerType });
      throw new Error(`Unsupported AI provider: ${providerType}`);
    }

    console.log("[AIService] Provider initialized successfully");
  }
  ...
  async generateRecommendations(
    ...
    try {
      ...
      logger.info("Calling AI provider for recommendations");
      logger.debug("AI prompt length", { length: prompt.length });

      const aiResponse = await this.provider.generateRecommendations(
        prompt,
        context,
        cancellationToken,
      );
      ...
      console.warn(
        `AI recommendations unavailable: ${errorMessage}. Falling back to raw analysis.`,
      );
  ...
  private buildPrompt(
    ...
    prompt += `Component: ${metadata?.componentName || "Unknown"}\n`;

    // Safely extract files analyzed
    const filesAnalyzed = metadata?.filesAnalyzed
      ? Object.values(metadata.filesAnalyzed)
          .filter((f) => f)
          .join(", ")
      : "No files specified";
    prompt += `Files analyzed: ${filesAnalyzed}\n\n`;
    ...
    if (cssIssues.length > 0) {
      ...
      cssIssues.forEach((issue, index) => {
        ...
        const cssSnippet = this.extractRelevantSnippet(
          codeSnippets.get("css") || "",
          issue.line,
          3,
        );
        if (cssSnippet) {
    ...
    if (tsIssues.length > 0) {
      ...
      tsIssues.forEach((issue, index) => {
        ...
        // Add relevant code snippet if available
        const tsSnippet = this.extractRelevantSnippet(
          codeSnippets.get("typescript") || "",
          issue.line,
          3,
        );
        if (tsSnippet) {
    ...
    if (templateIssues.length > 0) {
      ...
      templateIssues.forEach((issue, index) => {
        ...
        // Add relevant code snippet if available
        const htmlSnippet = this.extractRelevantSnippet(
          codeSnippets.get("html") || "",
          issue.line,
          3,
        );
        if (htmlSnippet) {
    ...
    prompt += `\n## Instructions\n\n`;
    prompt += `For EACH issue above, provide a concise recommendation with:\n`;
    prompt += `1. **Summary**: A clear problem statement (max 1 sentence, under 80 characters)\n`;
    prompt += `2. **Recommendation**: Actionable steps to fix (max 2 sentences, under 150 characters)\n`;
    prompt += `3. **Priority**: "high" (critical), "medium" (important), or "low" (minor)\n`;
    prompt += `4. **Code Example** (optional): Keep it SHORT (max 2 lines)\n\n`;
    prompt += `CRITICAL: Return ONLY a valid JSON array. NO markdown formatting, NO code blocks, NO backticks, NO explanations.\n`;
    prompt += `Start directly with [ and end with ]\n`;
    prompt += `Keep ALL text concise to avoid truncation.\n\n`;
    prompt += `[\n`;
    prompt += `  {\n`;
  ...
  private parseAIResponse(
    ...
    try {
      ...
      try {
        ...
        const recommendations = JSON.parse(jsonString) as AIRecommendation[];

        // Validate and sanitize recommendations
        return recommendations
          .filter(
            (rec) =>
              rec.category && rec.summary && rec.recommendation && rec.priority,
          )
          .map((rec, index) => ({
            ...rec,
        ...
        if (lastCompleteObject > 0) {
          ...
          const fixedJson =
            jsonString.substring(0, lastCompleteObject + 1) + "]";

          try {
            const recommendations = JSON.parse(fixedJson) as AIRecommendation[];
            console.log(
              `Successfully recovered ${recommendations.length} recommendations from truncated JSON`,
            );
